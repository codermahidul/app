<?php

class products
{
    private $Main, $Curls,$GetData;

    function __construct()
    {
        $this->Main = new Main;
        $this->Curls = new Curls;
        $this->GetData = new GetData;

        $this->Main->SetTitle("Product List");
        $this->Main->AddMetaDescription("Hairizer Product List");
        $Css = '<link rel="stylesheet" href="' . VIEW_PATH . 'css/WebSite/Common/Common.css?v='.VERSION.'">
        
        <link rel="stylesheet" href="'.VIEW_PATH.'tokeniz/tokenize2.css">';

        $Js = '<script type="module" src="' . VIEW_PATH . 'js/WebSite/List/Product.js?v='.VERSION.'"></script>';
        
        $Header = 'WebSite/Layout/Header.phtml';
        $Footer = 'WebSite/Layout/Footer.phtml';

        $this->Main->SetJs($Js);
        $this->Main->SetCss($Css);
        $this->Main->SetHeaders($Header);
        $this->Main->SetFooter($Footer);


        $data = [];
        $city_id="";
        if(isset($_COOKIE['rate_cityChoose'])){
            $city_id=$_COOKIE['rate_cityChoose'];
        }

        $pr1=["payment"=>"1","trash" => "0", "admin_confirm" => "1"];
        if($city_id !=="")
        {
            $pr1['city_id']=$city_id;
        }

        $data1 = $this->Curls->curl("product/getTop",$pr1 );
        if ($data1->status == "true") {
            $data = array_merge($data, $data1->data->data);
            /////set static product load
            $ids = [];
            foreach ($data as $ddd) {
                array_push($ids, $ddd->id);
            }

            $idNotIn = implode(",", $ids);
        }

        $pr2=["trash" => "0", "admin_confirm" => "1", "idNotIn" => $idNotIn];
        if($city_id !=="")
        {
            $pr2['city_id']=$city_id;
        }

        $data2 = $this->Curls->curl("product/get",$pr2 );
        $haveMore = false;
        if ($data2->status == "true") {
            $total = $data2->data->total;

            if ($total > '30') {
                $haveMore = true;
            }
            $data = array_merge($data, $data2->data->data);
        }
 
        $bigBanner=$this->GetData->GetBigBanner("product");


        $this->Main->CreateViewHeaderFooter(
            "WebSite/ListsP.phtml",
            [
                "data" => $data,
                "idNotIn" => $idNotIn,
                "haveMore" => $haveMore,
                "url_product" => "ProductDetail",
                "type_shows"=>"product",
                "title_page"=>"Product List",
                "category_search"=>"true",
                "have_price"=>"1",
                "have_rating"=>"1",
                "total"=>($total+$data1->data->total),
                "bigBanner"=>$bigBanner->data->data,
                "city_search"=>"true",
            ]
        );
    }
}
