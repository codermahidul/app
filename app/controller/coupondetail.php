...
class couponDetail 
{
    ...
    function __construct()
    {
        ...
        $Js='<script type="module" src="'.VIEW_PATH.'js/WebSite/List/CouponDetail.js?v='.VERSION.'"></script>
        ...
        $data=[];
        $relates=[];
        $response= $this->Curls->curl("coupon/get",["trash"=>"0","admin_confirm"=>"1","title_slug"=>$title_slug]);
        if($response->status=="true")
        {
            ...
            $relates = $this->Curls->curl("coupon/get", [
                "trash" => "0",
                "admin_confirm" => "1",
                "idNotIn" => $data[0]->id,
        ...
        $bigBanner=$this->GetData->GetBigBanner("couponDetail");
        $share=$this->GetData->CheckShare($data[0]->id,"coupon");

        $dd=$data[0];
        $clime=$this->GetData->GetClime($dd,"coupon");

        $this->Main->CreateViewHeaderFooter("WebSite/CouponDetails.phtml",[
        "data"=>$data,
        "report"=>$proof->data,
        "relates"=>$relates,
        "logged"=>$logged,
