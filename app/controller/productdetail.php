...
class productDetail 
{
    ...
    function __construct()
    {
        ...
        $Js='<script type="module" src="'.VIEW_PATH.'js/WebSite/List/ProductDetail.js?v='.VERSION.'"></script>
        ...
        $response= $this->Curls->curl("product/get",["trash"=>"0","admin_confirm"=>"1","title_slug"=>$title_slug]);
        if($response->status=="true")
        {
            ...
            $relates = $this->Curls->curl("product/get", [
                "trash" => "0",
                "admin_confirm" => "1",
                "idNotIn" => $data[0]->id,
        ...
        $bigBanner = $this->GetData->GetBigBanner("productDetail",$data[0]->cats_id);

        $share=$this->GetData->CheckShare($data[0]->id,"product");
        $dd=$data[0];
        $clime=$this->GetData->GetClime($dd,"product");

        $this->Main->CreateViewHeaderFooter("WebSite/ProductDetails.phtml",[
        "data"=>$data,
        "report"=>$proof->data,
        "relates"=>$relates,
        "logged"=>$logged,
