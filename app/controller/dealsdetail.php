...
class dealsDetail 
{
    ...
    function __construct()
    {
        ...
        $Js='<script type="module" src="'.VIEW_PATH.'js/WebSite/List/DealsDetail.js?v='.VERSION.'"></script>
        ...
        $data=[];
        $relates=[];
        $response= $this->Curls->curl("deals/get",["trash"=>"0","admin_confirm"=>"1","title_slug"=>$slug]);
        if($response->status=="true")
        {
            ...
            $relates = $this->Curls->curl("deals/get", [
                "trash" => "0",
                "admin_confirm" => "1",
                "idNotIn" => $data[0]->id,
        ...
        $bigBanner=$this->GetData->GetBigBanner("dealsDetail");

        $share=$this->GetData->CheckShare($data[0]->id,"deals");

        $dd=$data[0];
        $clime=$this->GetData->GetClime($dd,"deals");

        $this->Main->CreateViewHeaderFooter("WebSite/DealsDetails.phtml",[
        "data"=>$data,
        "report"=>$proof->data,
        "relates"=>$relates,
        "logged"=>$logged,
