...
class prizeDetail 
{
    ...
    function __construct()
    {
        ...
        $data=[];
        $relates=[];
        $response= $this->Curls->curl("prize/get",["trash"=>"0","admin_confirm"=>"1","idSearch"=>$idSearch]);
        if($response->status=="true")
        {
            ...
            $relates = $this->Curls->curl("prize/get", [
                "trash" => "0",
                "admin_confirm" => "1",
                "idNotIn" => $data[0]->id,
        ...
        $share=$this->GetData->CheckShare($idSearch,"prize");

        $dd=$data[0];
      

        $this->Main->CreateViewHeaderFooter("WebSite/PrizeDetails.phtml",[
        "data"=>$data,
        "report"=>$proof->data,
        "relates"=>$relates,
        "logged"=>$logged,
