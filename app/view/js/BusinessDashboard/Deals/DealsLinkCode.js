...
const DtClass=new DataTable("dataTable","couponAble/get_all_deals_link_code");
...
{
    BtnNew.onclick=()=>{
        SendDataForm(BtnNew,"newFrm","deals/setLinkCode",[LinkCode],[
...
DtClass.ClickTable("remove",(data)=>{
    SwalRemoveFetch("deals/removeLinkCode",{idSearch:GetIdFromUrl(),idSearchLink:data.id},()=>{
        DtClass.Refresh();
    })
})
...
BtnLinkCode.onclick=()=>{
    SendDataForm(BtnLinkCode,"linkFrm","deals/updateLinkCode",[Used,Link_Code],[],false,()=>{
        DtClass.Refresh();
