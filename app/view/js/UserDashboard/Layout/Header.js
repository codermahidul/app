...
const getMyNote = () => {
    ...
    Fetching("userAction/getMyNote", { is_read: 0,type_name:"user" }).then((data) => {
        if (data.status === "true") {
...
const ShowResult = () => {
    ...
    if(notes.length>0){
        ...
        notes.forEach(e => {
            ...
            str+='<a href="'+URLPATH+'MyNotification/'+e.id+'">';
        ...
        str+='<a class="btn btn-prim" href="'+URLPATH+'MyNotification">View All</a>';
        str += '</div>';
