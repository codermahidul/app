...
const DtClass=new DataTable("dataTable","couponAble/get_all_deals_for_business");
...
AddNewBtn.onclick=()=>{
    window.location.href=URLPATH+'AddDealsP'
}

DtClass.ClickTable("updateImage",(data)=>{
    imageShow(data.id,"deals/getAll")
})
...
DtClass.ClickTable("view",(data)=>{
    DealsC.ViewInfo("deals/getAll",{idSearch:data.id},()=>{$("#ModalView").modal("show")})
})
