...
const DtClass=new DataTable("dataTable","couponAble/get_all_deals_link_code_sub");
...
{
    BtnNew.onclick=()=>{
        SendDataForm(BtnNew,"newFrm","deals/setLinkCodeSub",[LinkCode],[
...
DtClass.ClickTable("remove",(data)=>{
    SwalRemoveFetch("deals/removeLinkCodeSub",{idSearch:GetIdFromUrl(),idSearchLink:data.id},()=>{
        DtClass.Refresh();
    })
})
...
const GetInfoEdit=(id)=>{
    ...
    return new Promise(resolve=>{
        Fetching("deals/getLinkCode",{idSearchLink:id}).then((data)=>{unblock();resolve(data)})
    })
}
...
BtnLinkCode.onclick=()=>{
    SendDataForm(BtnLinkCode,"linkFrm","deals/updateLinkCodeSub",[Used,Link_Code],[],false,()=>{
        DtClass.Refresh();
