...
const DtClass=new DataTable("dataTable","couponAble/get_all_coupon_link_code_sub");
...
{
    BtnNew.onclick=()=>{
        SendDataForm(BtnNew,"newFrm","coupon/setLinkCodeSub",[LinkCode],[
...
DtClass.ClickTable("remove",(data)=>{
    SwalRemoveFetch("coupon/removeLinkCodeSub",{idSearch:GetIdFromUrl(),idSearchLink:data.id},()=>{
        DtClass.Refresh();
    })
})
...
BtnLinkCode.onclick=()=>{
    SendDataForm(BtnLinkCode,"linkFrm","coupon/updateLinkCodeSub",[Used,Link_Code],[],false,()=>{
        DtClass.Refresh();
