...
///datatble
setDataTable("datatable", "adminPrizeTable/get_all", param, column);
...
const ShowModalEdit = (id) => {
    ...
    Fetching("prize/getDataAdmin", { idSearch: id }).then((data) => {
...
SaveEdit.onclick = () => {
    block();
    SendDataForm(SaveEdit, "frmEdit", "prize/updatePrizeAdmin", [title_e], [], true, Resets,false);
}

BtnDeleteImage.onclick = () => {
    RemoveImage("prize/deleteImageAdmin");
}

BtnSaveEditImage.onclick = () => {
    SetImages(BtnSaveEditImage,"prize/updateImageAdmin");
}
...
$('#m_table_1 tbody').on('click', 'a', function (e) {
    ...
    switch (data.action) {
        ...
        case "updateimage": {
            imageShow(data.id,"prize/getDataAdmin")
            break;
        }

        case "view": {

            ShowInfoDataDiv(data.id,"prize/getDataAdmin")
            break;
        }
