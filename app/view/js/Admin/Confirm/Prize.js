...
setDataTable("datatable", "prizetableadmin/get_all", param, column);
...
$('#m_table_1 tbody').on('click', 'a', function (e) {
    ...
    switch (data.action) {
        ...
        case "confirm": {
            ConfirmData("prize/confirmprize", { idSearch: data.id, admin_confirm: 1 },RefreshTable);

            break;
        }
        case "reject": {
            
            RejectData("prize/confirmprize", { idSearch: data.id, admin_confirm: 0 },RefreshTable);
            break
        }
        case "note":{
...
SaveNoteBtn.onclick=()=>{
    ...
    block();
    SaveNoteAdmin("noteFromAdmin",$("#itemid").val(),"prize",$("#text").val(),$("#to_user").val(),()=>{
        unblock();
        RefreshTable();
        $("#"+id).modal("hide");
    });
...
saveHomeShowBtn.onclick =()=>{
    HomeShowAction("fetchdata/removehomeshow",{type_table:"prize"},"user_business")
}
