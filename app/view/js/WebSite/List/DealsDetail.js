...
SaveReview.onclick = () => {
    ...
    let param = {
        text: val,
        deals_id: $("#deals_id").val(),
        
    };

    Fetching("deals/addNewComment", param).then((data) => {
...
SaveReply.onclick = () => {
    ...
    let urls = "deals/addReplyComment";
    if (type_user === "business") {
        urls = "deals/addReplyCommentBusiness";
...
SaveRate.onclick = () => {
    ...
    block();
    Fetching("deals/rate", formData).then((data) => {
        unblock();
        if (data.status == "true") {
...
const GetComments=()=>{
    CommentClass.showComment("deals/getComment", { deals_id:$("#deals_id").val() },AfterGetComment);
}
...
const AddLikeOnComment=(e)=>{
    ...
    CommentClass.AddLikeOnComment(id,"deals/addNewCommentLike",params,CommentClass.SetBtnAddComment,div,divNumber);
    CommentClass.SetBtnAddComment(CommentClass.AddReplyOnComment, AddLikeOnComment, RemoveLikeOnComment)
...
const RemoveLikeOnComment=(e)=>{
    ...
    let divNumber = $("#like_number_" + id);
    let params={ idSearch: id, creator_type: $("#type_creator").val() };
    CommentClass.AddLikeOnComment(id,"deals/removeCommentLike",params,CommentClass.SetBtnAddComment,div,divNumber);
    CommentClass.SetBtnAddComment(CommentClass.AddReplyOnComment, AddLikeOnComment, RemoveLikeOnComment)
